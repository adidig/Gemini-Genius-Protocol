...
> **English/Hebrew Note:** Full documentation is available in both languages. English starts here; the Hebrew version (with setup instructions) is located at the bottom of the page.
>
> **הערה למשתמשים:** הדוקומנטציה המלאה זמינה בשתי השפות. הגרסה האנגלית מופיעה כאן; הגרסה העברית (כולל הוראות הגדרה) נמצאת בתחתית העמוד.
...
Due to the structural nature of this protocol (which forces the entire response into a single code block for maximum speed), the standard "Copy" button cannot be rendered by the UI. These two features are technically incompatible. To copy text, please use manual selection.

### Why this Protocol is Genius
1. **Extreme Speed:** No UI overhead.
...
### ⚠️ מגבלה טכנית חשובה
שימו לב: בשל הארכיטקטורה של הפרוטוקול הזה (שמאלץ את כל התשובה להיכתב בתוך בלוק קוד אחד כדי להשיג מהירות מקסימלית), כפתור ה"העתקה" המובנה של הממשק לא יכול להיווצר. שני האלמנטים הללו אינם יכולים להתקיים יחד מבחינה טכנית. כדי להעתיק טקסט, יש לבצע סימון ידני.

### מה הופך את הפרוטוקול הזה לגאוני?
1. **מהירות עבודה מקסימלית:** התשובות נוצרות הרבה יותר מהר.
